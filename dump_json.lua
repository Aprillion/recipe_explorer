...
table_to_json = function(node, ignored_indexes, max_level, level, first, last, is_array)
  local out = ''
  local i = 0
  for k, v in pairs(node) do
    ...
    if is_array then
      out = out .. node_to_json(v, ignored_indexes, max_level, level)
    else
      out = out .. '"' .. k .. '": '
      if ignored_indexes[k] then
        ...
        out = out .. node_to_json(v, ignored_indexes_next, max_level, level)
      end
    end
  end
  if i > 0 then
...
node_to_json = function(node, ignored_indexes, max_level, level)
  level = level or 0
  local out = ''
  if type(node) == "table" then
    if #node > 0 then
      out = out .. table_to_json(node, ignored_indexes, max_level, level + 1, '[', ']', true)
    else
      if node.__self then
      ...
      out = out .. table_to_json(node, ignored_indexes, max_level, level + 1, '{', '}', false)
    end
...
game.write_file(dir .. "/game.json", node_to_json(game_settings, {}, 2))

local ignored_indexes = {force=true, prototype=true, group=true, subgroup=true}
game.write_file(dir .. "/game.player.force.recipes.json", node_to_json(game.player.force.recipes, ignored_indexes, 4))
game.write_file(dir .. "/game.player.force.technologies.json", node_to_json(game.player.force.technologies, ignored_indexes, 2))
